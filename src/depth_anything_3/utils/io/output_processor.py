...
class OutputProcessor:
    ...
    def __call__(self, model_output: dict[str, torch.Tensor]) -> Prediction:
        ...
        if "deformation" in model_output:
            deformation = self._extract_deformation(model_output)
        else:
            deformation = None
    ...
    def _extract_deformation(self, model_output: dict[str, torch.Tensor]) -> np.ndarray:
        """
        Extract deformation tensor from model output and convert to numpy.
        ...
        Returns:
            Depth array with shape (N, H, W)
        """
        deformation = model_output["deformation"].squeeze(0).squeeze(-1).cpu().numpy()  # (N, H, W)
        return deformation
